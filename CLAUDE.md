...
|------|---------|-------------|
| `edges.jsonl` | typed relationships (extends, contradicts, supports, inspired_by, tested_by, invalidates, supersedes, addresses_gap, derived_from) | `python tools/research_wiki.py add-edge` |
| `context_brief.md` | compressed context: claims + gaps + failed ideas + papers + edges (≤8000 chars) | `python tools/research_wiki.py rebuild-context-brief` |
| `open_questions.md` | open questions: under-supported claims + open questions from papers/topics | `python tools/research_wiki.py rebuild-open-questions` |
...
3. **System Python**: if neither exists, use `python` (Unix/macOS) or `python` (Windows) directly
