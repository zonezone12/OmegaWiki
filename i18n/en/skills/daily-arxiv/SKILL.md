...
1. Run fetch_arxiv.py to get the new paper list:
   ```bash
   python tools/fetch_arxiv.py --hours <hours> -o /tmp/arxiv_feed.json
   ```
2. Fetch DeepXiv trending papers (past 7 days):
   ```bash
   python tools/fetch_deepxiv.py trending --days 7 --limit 20
   ```
...
2. Load checkpoint (skip already-completed papers if one exists):
   ```bash
   python tools/research_wiki.py checkpoint-load wiki/ "daily-arxiv-{date}"
   ```
3. Take the first `--max-ingest` papers (default 5) and call `/ingest` one at a time:
   ...
   - After each success, record checkpoint:
     ```bash
     python tools/research_wiki.py checkpoint-save wiki/ "daily-arxiv-{date}" "{arxiv_id}"
     ```
   - On failure, mark and continue:
     ```bash
     python tools/research_wiki.py checkpoint-save wiki/ "daily-arxiv-{date}" "{arxiv_id}" --failed
     ```
4. If `--dry-run`, skip actual ingest; mark "would ingest" in the digest
5. After all done, clear checkpoint:
   ```bash
   python tools/research_wiki.py checkpoint-clear wiki/ "daily-arxiv-{date}"
   ```
...
2. Append digest to `wiki/log.md`:
   ```bash
   python tools/research_wiki.py log wiki/ "daily-arxiv | {N_ingested} ingested, {N_relevant} relevant / {N_total} total"
   ```
...
- **log.md is append-only**: use `python tools/research_wiki.py log` to append
...
### Tools（via Bash）
- `python tools/fetch_arxiv.py --hours <N> -o <path>` — pull arXiv RSS
- `python tools/fetch_deepxiv.py trending --days 7 --limit 20` — fetch trending papers
- `python tools/fetch_deepxiv.py brief <arxiv_id>` — fetch paper TLDR and keywords
