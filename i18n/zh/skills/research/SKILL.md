...
1. **解析输入**：
   ...
   - 生成 slug：`python tools/research_wiki.py slug "{direction}"`
...
**触发条件**：运行 `python tools/research_wiki.py maturity wiki/ --json`，若 `level == "cold"` 且 `papers < 3`：自动进入 Bootstrap。否则跳过，直接进入 Stage 1。
...
2. **搜索相关论文**（使用 Agent tool 并行 3 路搜索）：
   ...
   - arXiv：`python tools/fetch_arxiv.py`（使用 direction 关键词）
...
### Tools（via Bash）
- `python tools/research_wiki.py slug "{title}"` — 生成 pipeline slug
- `python tools/research_wiki.py set-meta <path> <field> <value>` — 更新 pipeline-progress 字段
- `python tools/research_wiki.py log wiki/ "<message>"` — 追加日志
- `python tools/research_wiki.py maturity wiki/ --json` — 检查 wiki 成熟度（Stage 0 触发条件 + Growth Report）
- `python tools/research_wiki.py init wiki/` — 初始化 wiki 结构（Stage 0）
- `python tools/fetch_deepxiv.py search "{query}" --mode hybrid --limit 20` — DeepXiv 语义搜索（Stage 0）
- `python tools/fetch_s2.py search "{query}" --limit 20` — Semantic Scholar 搜索（Stage 0）
- `python tools/fetch_arxiv.py` — arXiv RSS 搜索（Stage 0）
