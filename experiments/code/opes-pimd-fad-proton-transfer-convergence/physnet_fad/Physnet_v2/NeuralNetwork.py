...
class PhysNet_v2(tf.keras.Model):
    ...
    def restore(self, load_file):
        ''' Load a model — uses path-based restore to work around TF2 object-graph
            tracking issues with tf.name_scope-wrapped tf.Variable attributes. '''
        ...
        loaded = 0
        for key, _ in tf.train.list_variables(load_file):
            if '.ATTRIBUTES/VARIABLE_VALUE' not in key:
                continue
            attr_path = key.replace('/.ATTRIBUTES/VARIABLE_VALUE', '')
            if not attr_path.startswith('model/'):
                continue
            parts = attr_path[6:].split('/')
            try:
                var = _resolve(self, parts)
                if isinstance(var, tf.Variable):
                    var.assign(tf.cast(ckpt.get_tensor(key), var.dtype))
                    loaded += 1
            except Exception:
                pass
        return loaded
